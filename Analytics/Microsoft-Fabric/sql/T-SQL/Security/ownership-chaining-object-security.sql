...
IF EXISTS(SELECT 1 FROM sys.tables WHERE name ='Product' AND schema_id=schema_id('demo'))
...
REVOKE SELECT ON OBJECT::demo.Product FROM [johndoe@yourdomain.com];  
GO
...
DENY SELECT ON [demo].[Product] TO [johndoe@yourdomain.com];
GO
...
/*
    ...
    **********************************************
     **Definition**
     Ownership chaining occurs when a user accesses an object (like a stored procedure) 
     that references another object (like a table). 
     If both objects have the same owner, SQL Server checks permissions only on the first object

     **Security Implications**    
     While ownership chaining can simplify permission management, 
     it can also pose security risks if not managed carefully. 
     It’s important to ensure that only trusted users have access to objects involved in ownership chains
*/
CREATE PROC [demo].[usp_TotalProductByCategory]
AS
...
GRANT EXECUTE ON [demo].[usp_TotalProductByCategory] TO [johndoe@yourdomain.com]
GO



EXECUTE [demo].[usp_TotalProductByCategory]
GO

--Clean Up
DROP PROC [demo].[usp_TotalProductByCategory]
GO
DROP TABLE [demo].[Product]
GO
DROP SCHEMA [demo]
