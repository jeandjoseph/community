...
/*==========================================================
SQL Server version of:
- products table
- products_vector table (embedding stored as JSON)
==========================================================*/
...
-- 2) Embeddings table (store the embedding array as JSON text)
CREATE TABLE dbo.products_vector
(
    productid       int NOT NULL,
    productname_embedding_json  nvarchar(max) NULL,  -- store embedding as JSON array: [0.12, -0.34, ...]
    CONSTRAINT PK_products_vector PRIMARY KEY (productid),
    CONSTRAINT FK_products_vector_products
...
-- verify table creation
SELECT TOP(5) * FROM dbo.product_reviews;
...
CREATE TABLE dbo.product_review_summary
(
    productid            int NOT NULL,
    abstractive_summary  nvarchar(max) NULL,   -- summarized review text
    sentiment_label      nvarchar(50) NULL,    -- 'positive', 'neutral', 'negative'
    positive_score       float(53) NULL,       -- confidence score
    neutral_score        float(53) NULL,       -- confidence score
    negative_score       float(53) NULL,       -- confidence score
    created_at           datetime2(0) NOT NULL
        CONSTRAINT DF_product_review_summary_created_at DEFAULT (sysdatetime()),

    CONSTRAINT PK_product_review_summary PRIMARY KEY (productid),
    CONSTRAINT FK_product_review_summary_products
        FOREIGN KEY (productid) REFERENCES dbo.products(productid)
);
GO

-- verify table creation and data will populate later after we run the summarization and sentiment analysis in Python
SELECT * FROM dbo.product_review_summary;